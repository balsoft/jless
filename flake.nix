...
# and is easy to include in a machine or user environment via the overlay.
{
  ...
  outputs = { self, nixpkgs, utils }:
    let
      ...
      # How to build
      build = { rustPlatform, python3, xorg, wayland }:
        rustPlatform.buildRustPackage {
          ...
          cargoLock = {
            lockFile = ./Cargo.lock;
            outputHashes."clipboard-0.5.0" = "sha256-FXw0eWBpVogyV/mHIpfhq9MmSqL5E2hg3tH0BoDfpHQ=";
          };
          nativeBuildInputs = [ python3 ];
          buildInputs = [ xorg.libxcb wayland ];
          doCheck = true;
        };

      # Create overlay that includes this package
      overlay = final: prev: { ${name} = final.callPackage build { }; };
      overlays = [ overlay ];
